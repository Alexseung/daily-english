  ...
  try {
    ...
    const results = await Promise.all(
      users.map(async (user) => {
        ...
        console.log(
          `📤 Sending test email to ${user.email} → ${firstContent.content}`
        );

        return resend.emails.send({
          from: "dailyenglish@stepinenglish.co.kr",
          to: user.email,
          subject: `Test: ${firstContent.content}`,
          react: React.createElement(Announcement, {
            item: {
              id: firstContent.id,
              content: firstContent.content,
              meaning: firstContent.meaning,
              meaningInKorean: firstContent.meaningInKorean,
              literalTranslation: firstContent.literalTranslation,
              sentences: firstContent.sentences,
            },
          }),
        });
      })
    ...
    console.error("❌ 테스트 이메일 전송 오류:", err);
    return new Response("테스트 이메일 전송 실패", { status: 500 });
  }
}
